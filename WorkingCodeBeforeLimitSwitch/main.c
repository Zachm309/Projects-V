...
int main(void)
{
	...
	Encoder_Init();
	ultraSonicInit();
	UART3printf("UART Test: Hello, UART is working!\n");
	LCDprintf("RS232 Test");
	...
	char receivedChar;
//		while (1) {
//        // Wait for a key press from Tera Term (received via UART)
//        receivedChar = UART3getcND();  // Wait until a character is received

//        // Echo the received character back to Tera Term
//        UART3putc(receivedChar);
//				UART3printf("\nYour character is %c\n", receivedChar);
//    }
...
for(;;) {
   ...
   // inputCommand = UARTgetcND();
		receivedChar = UART3getcND();
		inputCommand = receivedChar;
	  UARTprintf("\nThe reveived char is: %c\n", inputCommand);
